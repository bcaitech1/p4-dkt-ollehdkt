...
class GeneralizedSaintPlus(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.cont_cols_cnt = len(self.args.cont_cols)
        ...
        self.enc_comb_proj = nn.Sequential(
                                    ...
                                    (len(self.args.cate_cols)), (self.hidden_dim//2)*(len(self.args.cate_cols))),
        ...
        self.dec_comb_proj = nn.Sequential(
                                    nn.Linear((self.hidden_dim//2)*
                                    (len(self.args.cate_cols)+1), (self.hidden_dim//2)*(len(self.args.cate_cols))),
                                    # nn.LayerNorm(self.hidden_dim)
                            )

        # Positional encoding
        self.pos_encoder = PositionalEncoding((self.hidden_dim//2)*(len(self.args.cate_cols)), self.dropout, self.args.max_seq_len)
        self.pos_decoder = PositionalEncoding((self.hidden_dim//2)*(len(self.args.cate_cols)), self.dropout, self.args.max_seq_len)
        ...
        self.transformer = nn.Transformer(
            d_model=(self.hidden_dim//2)*(len(self.args.cate_cols)),
        ...
        self.fc = nn.Linear((self.hidden_dim//2)*(len(self.args.cate_cols)), 1)
    ...
    def forward(self, input):
        ...
        del input
        # Embedding(Encoder)
        embec_interaction = self.embedding_interaction(interaction)

        embed_cate_col = [self.embedding_cate[i](cate_cols[i]) 
        for i in range(len(cate_cols))]

        # 연속형 - 각자 다른 layer를 거친 후 concat
        # (batch, max_seq_len, 1)
        embed_cont_tmp = [self.embedding_cont(v) for i,v in enumerate(other_cont)]
        ...
        cat_list.append(embec_interaction)
        ...
        out = out.contiguous().view(batch_size, -1, (self.hidden_dim//2)*(len(self.args.cate_cols)))
        out = self.fc(out)
...
class GeneralizedSaint(nn.Module):
    ...
    def forward(self, input):
        ...
        # cat_list.extend(embed_cate_col)
        cat_list = list(embed_cate_col)
        cat_list.append(embec_interaction)
